...
| --- | --- | --- | --- |
| Product surface | B | The first product surface is defined as CPU-efficient face image inference for gender and numeric age, with race explicitly out of scope. | Add a dedicated inference/runtime contract before integrating a serving surface. |
| Architecture docs | B | FastFace model, dataset, GPU training, ONNX export, HF release, and CPU benchmark docs describe the phase-1 architecture. | Add a single serving contract covering preprocessing, output schema, batching, and CPU thread policy. |
